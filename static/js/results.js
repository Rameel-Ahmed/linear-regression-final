...
sklearnEquation.innerHTML = `
    ...
    <div class="sklearn-metrics">
        <div class="metric-item">
            <div class="metric-value">${sklearnModel.metrics?.r2?.toFixed(4) || 'N/A'}</div>
            <div class="metric-label">Sklearn R²</div>
        </div>
        <div class="metric-item">
            <div class="metric-value">${sklearnModel.metrics?.rmse?.toFixed(4) || 'N/A'}</div>
            <div class="metric-label">Sklearn RMSE</div>
        </div>
        <div class="metric-item">
            <div class="metric-value">${sklearnModel.metrics?.mae?.toFixed(6) || 'N/A'}</div>
...
function displayModelSummary() {
    try {
        ...
        if (comprehensiveResults.sklearnResults && comprehensiveResults.sklearnResults.sklearn_results) {
            ...
            if (sklearnR2) {
                sklearnR2.textContent = sklearnModel.metrics?.r2 ? sklearnModel.metrics.r2 : 'N/A';
            }

            const sklearnRmse = document.getElementById('sklearnRmse');
            if (sklearnRmse) {
                sklearnRmse.textContent = sklearnModel.metrics?.rmse ? sklearnModel.metrics.rmse : 'N/A';
            }

            const sklearnMae = document.getElementById('sklearnMae');
            if (sklearnMae) {
                sklearnMae.textContent = sklearnModel.metrics?.mae ? sklearnModel.metrics.mae : 'N/A';
            }
