    ...
    Plotly.newPlot(container, traces, layout, config).then(() => {
        ...
        container.on('plotly_click', function(data) {
            ...
            if (point.curveNumber === 0) {
                const x = point.x;
                const y = point.y;
                console.log(`Clicked point: ${x_column}=${x.toFixed(3)}, ${y_column}=${y.toFixed(3)}`);
                
                // Highlight the clicked point
                const update = {
                    'marker.size': Array(x_values.length).fill(6).map((size, i) => 
                        Math.abs(x_values[i] - x) < 0.001 && Math.abs(y_values[i] - y) < 0.001 ? 12 : 6
                    )
                };
...
export function createRangePlot() {
    ...
    const trace = {
        ...
        hovertemplate: 
            `<b>%{x}</b><br>` +
            `<b>Range</b>: %{y:.3f}<br>` +
            `<b>Min</b>: ${x_column === '%{x}' ? xMin.toFixed(3) : yMin.toFixed(3)}<br>` +
            `<b>Max</b>: ${x_column === '%{x}' ? xMax.toFixed(3) : yMax.toFixed(3)}<br>` +
            `<extra></extra>`
    };
